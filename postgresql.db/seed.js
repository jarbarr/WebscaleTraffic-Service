...
const createProperties = (callback) => {
  ...
  for (let i = 1; i <= 10; i += 1) {
    ...
    let total = nightly_fee + cleaning_fee + service_fee;

    const queryString = `INSERT INTO properties (nightly_fee, rating, reviews, minimum_stay, maximum_guest, cleaning_fee, service_fee, total) VALUES (${nightly_fee}, ${rating}, ${reviews}, ${minimum_stay}, ${maximum_guest}, ${cleaning_fee}, ${service_fee}, ${total})`;
...
/* eslint-disable */
let createReservations = (propertyID, minimumStay) => {
  ...
  let numOfReservation = faker.random.number({ min: 1, max: 20 });
