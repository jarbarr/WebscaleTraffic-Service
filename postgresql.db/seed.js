...
const createProperties = (callback) => {
  ...
  for (let i = 1; i <= 10; i += 1) {
    ...
    if (rating >= 3) {
      service_fee = Math.fround(faker.random.number({ min: 80, max: 150 }));
    } else if (rating < 3) {
      service_fee = Math.fround(faker.random.number({ min: 20, max: 80 }));
    }
    cleaning_fee = Math.fround(0.03 * (nightly_fee + service_fee)); // 3% of subtotal
...
let createReservations = (propertyID, minimumStay, subtotal, guests) => {
  ...
  let numOfReservation = faker.random.number({ min: 5, max: 15 });
  ...
  for (let i = 0; i < numOfReservation; i += 1) {
    ...
    dates[propertyID] = [lengthOfStay, booking_id, randomDate, total];
  }
  /* eslint-disable */
  for (let property in dates) {
    for (let j = 0; j < dates[property][0]; j += 1) {
      ...
      let queryString = `INSERT INTO reservations (booking_id, property_id, booked_date, total, guests) VALUES (${rez}, ${property}, '${booking}', ${sum}, ${guests})`;
      db.query(queryString, (error) => {
        if (error) {
...
//       } else {
//         Console.log(`Success! Property ${i} inserted into properties table:`, nightly_fee, rating, reviews, minimum_stay, maximum_guest, cleaning_fee, service_fee, subtotal);
//         propertyData[i] = [minimum_stay, subtotal, guests];
//       }
//     });
//   }
// };
...
// //  insert into reservations table
// /* eslint-disable */
// let createReservations = (data) => {
//   // create an array to store the all booked_dates for the input roomID
//   // propertyID, minimumStay, subtotal, guests
//   let dates = {};
//   // generate a random number of reservations between 1 and 15 for the input roomID
//   let numOfReservation = faker.random.number({ min: 5, max: 15 });
//   // iterate over the number of reservations
//   for (let property in propertyData) {
//     let booking_id = faker.random.number({ min: 10000, max: 20000 });
//     // generate a random number of stay between minimumStay and 10 for the current reservation
//     let lengthOfStay = faker.random.number({ min: 0, max: 10 }) + minimumStay;
//     const today = moment(new Date).format('YYYY-MM-DD');
...
//       let last = randomDate;
//       let bookedDate = moment(last).add(j, 'days').format('YYYY-MM-DD');

//       // declare query string
//       let queryString = `INSERT INTO reservations (booking_id, property_id, booked_date, total, guests) VALUES (${booking_id}, ${property}, '${bookedDate}', ${total}, ${guests})`;
//       db.query(queryString, (error) => {
//         if (error) {
//           Console.log(`Failed to insert data to reservations table where property_id = ${property}: `, error, property, rez, booking, typeof(booking));
//         } else {
...
// // invoke generateRooms function
// // createProperties()
// createReservations(propertyData);
// // createReservations();